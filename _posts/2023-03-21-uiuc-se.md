...
After that, I clone the code to local and try to fix it. Here is the result of `nodex` in 10 times:
...
With patients, I think that the `pull request in github` may give me some ideas. So I search `flaky` [in](https://github.com/dromara/hutool/pull/1270) closed requests, and I found a useful results:
...
This fix request shows that the reason of the failure because of the Non-Deterministic return value. So I think problem maybe mainly happened on `String` and `List`. Here is the sourse code of this test:
`cn.hutool.core.annotation.AnnotationUtilTest#getCombinationAnnotationsWithClassTest`
...
Seems like no clue, so I back to the paper

# Paper

![1679542966083](../images/2023-03-21-uiuc-se/1679542966083.png)

Here is how `nodex` test shuffle the order:
![1679966006091](../images/2023-03-21-uiuc-se/1679966006091.png)

![1679966017190](../images/2023-03-21-uiuc-se/1679966017190.png)

So, it is probably cause by hashmap() or other order function. But it didn't work, it seems that unit test `getCombinationAnnotationsWithClassTest` didn't use hashmap. Actually, the failure shows more details:`expected:<[??]> but was:<[repeat-annotation]>`

**`repeat-annotation`** is important. Here is the code:
