...
State transitions: $s_{t+1}=f(s_t, a_t)$, or stochastic $s_{t+1}\sim P(\cdot\mids_t, a_t)$
...
This describes in $s$, how taking $a$ is better than taking random $\pi(\cdot\mids)$.
